...
- stdio 서버 모드
- 3개 MCP 도구 제공 (v2.0: add_cli 추가)
- **Status**: ✅ Implemented

**FR-002**: CLI 목록 조회 (list_available_clis)
...
**FR-003**: CLI 메시지 전송 (send_message)
...
**NFR-001**: 응답 시간
- `list_available_clis`: <2초 **(실제: <1초)** ✅
- `send_message` 에러 응답: <0.5초 **(실제: <0.5초)** ✅
