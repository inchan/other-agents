...
async def test_bot():
    ...
    await call_tool("add_agent", {"name": "test-bot", "command": "cat"})
    return "test-bot"
...
class TestSyncAsyncScenarios:
    ...
    async def test_sync_basic_query(self, test_bot):
        ...
        result = await call_tool(
            "use_agent", {"cli_name": test_bot, "message": "Hello Sync", "run_async": False}
        )
    ...
    async def test_async_task_start(self, test_bot):
        """[Async] 비동기 작업 시작"""
        result = await call_tool(
            "use_agent", {"cli_name": test_bot, "message": "Hello Async", "run_async": True}
        )
    ...
    async def test_async_task_polling(self, test_bot):
        ...
        # 비동기 작업 시작
        result = await call_tool(
            "use_agent", {"cli_name": test_bot, "message": "Hello Async Polling", "run_async": True}
        )
    ...
    async def test_async_parallel_tasks(self, test_bot):
        ...
        for i in range(3):
            tasks.append(
                call_tool(
                    "use_agent",
                    {"cli_name": test_bot, "message": f"Parallel {i}", "run_async": True},
                )
            )
    ...
    async def test_async_unknown_cli_error(self):
        ...
        result = await call_tool(
            "use_agent", {"cli_name": "unknown_bot", "message": "test", "run_async": True}
        )
        task_id = result["task_id"]
