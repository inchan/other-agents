...
class TestFileHandlerCoverage:
    ...
    def test_execute_with_session_success(
        self, mock_file_open, mock_close, mock_fdopen, mock_mkstemp,
        mock_cleanup, mock_execute_cli, mock_is_installed, mock_registry, mock_session_manager
    ):
        """세션 모드 실행 성공 테스트"""
        # Setup
        ...
        # Verify
        mock_session_manager.return_value.create_or_get_session.assert_called_with("user-session-1", "claude")
    ...
    def test_execute_with_session_yaml_prompt(
        self, mock_yaml_dump, mock_file_open, mock_close, mock_fdopen, mock_mkstemp,
        mock_cleanup, mock_execute_cli, mock_is_installed, mock_registry, mock_session_manager
    ):
    ...
    def test_execute_cli_file_based_claude_system_prompt(
        self, mock_file_open, mock_close, mock_fdopen, mock_mkstemp,
        mock_cleanup, mock_execute_cli, mock_is_installed, mock_registry
    ):
        # Setup
        mock_registry.return_value.get_all_clis.return_value = {
            "claude": {"command": "claude", "timeout": 30}
    ...
    def test_validate_and_filter_args_complex(self):
        config = {
            "supported_args": ["--model", "--verbose", "-n"]
        }
